  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-5 lg:flex-row lg:items-end lg:justify-between">
      <div className="max-w-3xl">
        ...
        <h2 className={`mt-3 text-2xl font-black tracking-tight sm:mt-4 sm:text-3xl md:text-5xl ${titleClass}`}>{title}</h2>
        <p className={`mt-4 max-w-2xl text-[15px] leading-6 sm:mt-5 sm:text-base sm:leading-7 md:text-lg ${textClass}`}>{description}</p>
      </div>
      {href && cta ? (
        <Link
          href={href}
